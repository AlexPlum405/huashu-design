...
**HTML是工具，但你的媒介和产出形式会变**——做幻灯片时别像网页，做动画时别像Dashboard，做App原型时别像说明书。**根据任务embody对应领域的专家**：
- **动画师**：精通节奏、缓动与镜头语言。
- **UX 设计师**：专注流程、交互细节与可用性。
- **幻灯片设计师**：擅长排版、视觉层次与叙事流。
- **空间设计师 (v11 新增)**：专注 3D 深度、玻璃拟态与 Spatial UI (Vision Pro 风格)。
- **创意编程师 (v11 新增)**：擅长生成艺术、数学美学与粒子系统。
- **桌面架构师 (v12 新增)**：精通 macOS (Sequoia) 与 Windows 11 (Fluent) 原生架构。
...
- **桌面应用原型 (v12 强化)**：生产级 macOS 与 Windows 11 客户端设计
- **Spatial UI / 3D 原型**：Vision Pro 风格的深度交互演示
- **设计变体探索**：并排对比多个设计方向，或用Tweaks实时调参
- **演示幻灯片**：1920×1080的HTML deck，可以当PPT用
- **动画Demo**：时间轴驱动的motion design，做视频素材或概念演示
- **创意编程/生成艺术**：动态粒子背景、数学驱动的视觉交互
...
> **任何涉及具体产品/技术/事件/人物的存在性、发布状态、版本号、规格参数的事实性断言，第一步必须用 `WebSearch` 能力验证，禁止凭训练语料做断言。**
...
1. 用 `WebSearch` 搜产品名 + 最新时间词（"2026 latest"、"launch date"、"release"、"specs"）
...
#### 1.a 核心资产协议 · v12.1 Expert 版 (涉及具体品牌时强制执行)

> **这是生产级设计的生命线。** Agent 是否走通这个协议，直接决定输出质量是 65 分还是 90 分。桌面架构师不仅要找 Logo，还要区分「品牌灵魂」与「系统默认」。

##### 5 步硬流程 (绝不可静默跳过)

**Step 1 · 问**：一句话问清楚：「这个品牌有 brand guidelines 吗？有的话直接给我，没有的话我去搜。」

**Step 2 · 搜官方品牌页**：
- 典型路径：`<brand>.com/brand` | `brand.<brand>.com` | `<brand>.com/press`。这些页面藏着最权威的 logo 和色值。
- **桌面/App 专项**：`<brand>.com/download` 获取客户端 Hero 渲染图；`App Store / Windows Store` 详情页获取标准 App Icon 和最新 UI。

**Step 3 · 下载资产 · 三条兜底路径**：
2026 年官网大多是 Next.js SSR inline SVG，或 Cloudflare 反爬。按成功率递减：
1. **独立 SVG 文件** (最理想)。
2. **官网 HTML 全文**：inline SVG 会藏在里面。cURL 源码并 grep `<svg>` 提取。
3. **产品截图取色**：获取不到矢量时，通过产品截图取色。403 错误时尝试 `curl -A "Mozilla/5.0"`。

**Step 4 · grep 提取真实色值与语义过滤**：
- **grep 提取**：绝对不能从记忆里猜。从下载资产里抓取所有 `#xxxxxx` 的 6 位 Hex，按频率排序，过滤黑白灰。
- **语义过滤 (New)**：剔除系统默认色（macOS `systemBlue` #007AFF, Win11 `LinkColor` #0067C0）。**只保留真正属于品牌的差异化色值**。
- **区分环境色**：产品截图里常有用户演示的其他品牌色（如用 Slack 演示喜茶红），那不是该工具本身的色。同时出现两种强色时必须区分。

**Step 5 · 固化为 `brand-spec.md` + `.json`**：
- 前四步抽完的色值，如果只留在内存里，下一页就忘。必须写一份 spec 文件到项目根目录，让所有 CSS 变量引用。
- HTML 只引用 `brand-spec.json` 中的变量，禁止临场发明颜色。
...
| **每个 8/10 分以上** | 不够 8 分**宁可不用**，用诚实 placeholder（灰块+文字标签）或 `image_generation` 以官方参考为基底生成 | 凑数 7 分素材进 brand-spec.md |
...
##### Step 5 · 固化为 `brand-spec.md` + `brand-spec.json` 文件（模板必须覆盖所有资产）

`brand-spec.md` 给人读，记录来源、判断、禁区和设计理由；`brand-spec.json` 给机器读，所有 HTML / 脚本都从这里注入 CSS variables 和真实资产路径。两者必须同时生成，缺一不可。
完整结构见 `references/brand-spec-structured.md`。

最小要求：
- `brand-spec.md` 必须记录来源、资产完整度、质量判断、禁区、气质关键词。
- `brand-spec.json` 必须包含 `assets.logo`、实体产品的 `assets.product`、数字产品的 `assets.ui`、`colors`、`fonts`。
- HTML 必须从 `brand-spec.json` 引用真实资产和注入 CSS variables。

**写完 spec 后的执行纪律（硬要求）**：
- 所有 HTML 必须**引用** `brand-spec.json` 里的资产文件路径，不允许用 CSS 剪影/SVG 手画代替
...
| **产品图（实体产品）找不到** | 优先 `image_generation` 以官方参考图为基底生成 → 次选向用户索取 → 最后才是诚实 placeholder（灰块+文字标签，明确标注"产品图待补"） |
...
| **CSS 剪影/SVG 手画代替真实产品图** | 生成的就是「通用科技动画」——黑底+橙 accent+圆角长条，任何实体产品都长一样，品牌识别度归零（DJI Pocket 4 实测 2026-04-20）| **几乎没有**——先走核心资产协议找真实产品图；真没有时用 `image_generation` 以官方参考图为基底生成；实在不行标诚实 placeholder 告诉用户"产品图待补" |
...
|-------------|--------------|
| HTML 型 | 生成完整 HTML → 截图 |
| AI 生成型 | `image_generation` 用风格 DNA + 内容描述 |
| 混合型 | HTML 布局 + AI 插画 |
...
**真实素材优先原则**（涉及用户本人/产品时）：
1. 先查当前 agent / 用户配置的**私有 memory 路径**下的 `personal-asset-index.json`
...
这个 skill 的产品价值是**帮助用户把握设计方向**，不是把一句 prompt 直接变成一个看似完整的成品。除非用户明确要求「只要策略/只要建议/不要产物」，任何设计任务都必须产出可见 artifact，但 artifact 之前必须先完成完整流程里的方向对齐。

**硬规则**：
- 不允许收到一句模糊 prompt 后直接写最终 HTML / PPT / 动画 / 原型。
- 不允许用"我先做一个看看"绕过问题清单、资产自检、方向重述和 Junior pass。
- 不允许把检查点降级为可选项。碰到 🛑 就停下等用户确认。
- 用户说"直接做"时，也只能进入完整流程的最小版本：先写 assumptions、列 3 个设计方向、标明需要用户确认的关键选择；确认后再进入 Full pass。

### 任务路由器 + Definition of Done

接到任务后，先读 `references/task-router.md`：判断输出类型、必读 reference、必用 assets/scripts、完成定义。路由器不是捷径，只是帮助完整流程走对分支。

### 标准流程（用TaskCreate追踪）

1. **理解需求**：
   - 🔍 **0. 事实验证（涉及具体产品/技术时必做，优先级最高）**：任务涉及具体产品/技术/事件（DJI Pocket 4、Gemini 3 Pro、Nano Banana Pro、某新 SDK 等）时，**第一个动作**是用 `WebSearch` 验证其存在性、发布状态、最新版本、关键规格。把事实写入 `product-facts.md`。详见「核心原则 #0」。**这步做在问 clarifying questions 之前**——事实错了问什么都歪。
...
3. **Reference Intake 自动采样**：按 `references/task-router.md` 的 Reference intake defaults 自动尝试公开参考源，详见 `references/reference-intake.md` + `references/inspiration-sources.md`。
   - 只采公开可访问内容 / 搜索摘要 / 用户给的链接或截图；不绕登录、不抓付费内容、不批量下载图片。
   - 产物是项目内 `reference-board.md`，用于记录 sources attempted、patterns extracted、do not copy。
   - 失败不阻塞：提示一句「部分来源需要登录或不可自动读取。你可以手动发截图/链接，也可以跳过；我会用已有公开参考继续。」然后继续流程。
4. **先答四问，再规划系统**：**这一步的前半段比所有 CSS 规则更决定输出**。
...
5. **构建文件夹结构**：`项目名/` 下放主HTML、需要的assets拷贝（不要bulk copy >20个文件）。
6. **Junior pass**：HTML里写assumptions+placeholders+reasoning comments。
   🛑 **检查点3：尽早show给用户（哪怕只是灰色方块+标签），等反馈再写组件**。
7. **Full pass**：填placeholder，做variations，加Tweaks。做到一半再show一次，不要等全做完。
...
12. **（核心）记录进化日志**：每当完成一个重大设计（Variation 对比、App 原型、动画成品）或执行完 Step 11 的专家评审后，**必须**将结果追加到 `results.tsv`。这是技能进化的「棘轮」。
    - 文件路径：`results.tsv`（位于 skill 根目录或项目 `.gemini/skills/huashu/`）
    - 格式：`timestamp \t commit \t task_type \t score \t status \t note \t eval_mode`
    - `task_type`：landing / deck / motion / app-prototype / design-advisor
    - `score`：Step 11 的总分（0-100），若未跑 Step 11 则记 `-`
    - `eval_mode`：`expert` (跑了Step 11) / `self` (自评) / `user` (用户反馈)
...
| 用户拒绝回答问题清单 | 用户说"不要问了，直接做" | 不直接生成成品。改走**压缩版完整流程**：列 assumptions + 3 个方向 + 关键风险，请用户确认一个方向后再 Full pass |
...
| 时间紧迫要快交付 | 用户说"30分钟内要" | 缩短问题清单和资产搜索范围，但不跳过 Junior pass / 方向确认 / lint / verify。交付时标注哪些检查是压缩执行 |
...
| `spatial_canvas.jsx` | 做 Spatial UI / 3D 原型 | 3D 场景 + 悬浮窗 + 凝视反馈 |
| `particles_engine.jsx` | 做创意编程 / 粒子背景 | Canvas 粒子系统组件 |
| `desktop_design_system.jsx` | 做生产级桌面应用 | macOS Sequoia / Windows 11 Fluent 核心组件库 |
| `templates/` | Junior Designer pass 起手模板 | landing / app overview / app flow / deck showcase / motion stage / infographic 的 assumptions + placeholder 骨架。只能作为完整流程的早期 artifact，不能跳过方向确认直接当成品 |
| `scripts/design_lint.py` | 交付前设计 lint | 检查 assumptions、真实资产引用、AI slop 风险、brand-spec.json 使用情况 |
...
| 任务路由器、强制契约、Definition of Done | `references/task-router.md` |
| Reference Intake 自动采样 | `references/reference-intake.md` + `references/inspiration-sources.md` |
| 结构化品牌规范 | `references/brand-spec-structured.md` |
| 反AI slop、内容规范、scale | `references/content-guidelines.md` |
| React+Babel项目setup | `references/react-setup.md` |
| 使用 Junior pass 起手模板 | `templates/README.md` + 对应 `templates/*.html` |
...
| 输出完后验证 | `references/verification.md` + `scripts/design_lint.py` + `scripts/verify.py` |
| Demo 源码参考索引 | `references/demo-index.md` |
| 资产保留 / 清理规则 | `references/asset-inventory.md` |
...
| 跨 Agent 环境能力映射 | `references/environment-adapters.md` |

## 跨 Agent 环境适配说明

本 skill 设计为 **agent-agnostic**——Claude Code、Codex、Cursor、Trae、OpenClaw、Hermes Agent 或任何支持 markdown-based skill 的 agent 都可以使用。统一用 capability 名称（`WebSearch` / `asset_fetch` / `image_generation` / `playwright_verify` / `design_lint` / `video_render` / `document_export`）思考，再按 `references/environment-adapters.md` 映射到当前 agent 的实际工具。
...
- **事实验证先于假设**（核心原则 #0）：涉及具体产品/技术/事件（DJI Pocket 4、Gemini 3 Pro 等）必须先用 `WebSearch` 验证存在性和状态，不凭训练语料断言。
