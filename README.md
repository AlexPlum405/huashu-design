...
但它也不是「一句话直接出成品」的魔法按钮。huashu-design 默认强制完整设计流程：先路由任务、问清关键问题、收集资产、给出方向和 Junior pass，再进入高保真制作。用户说「直接做」时，也会先给 assumptions 和 3 个方向让你确认，避免 agent 把错误方向做得很精美。
...
huashu-design v2.1 开始把「完整流程」当成硬约束，而不是建议：

- **任务路由器**：先判断是网页、App、PPT、动画、信息图还是评审，再读取对应 reference 和 starter assets。
- **强制方向确认**：禁止从一句模糊 prompt 直接生成最终成品；必须先给 assumptions、方向和 placeholder。
- **Reference Intake**：默认自动尝试公开参考源，提炼布局、层级、密度、动效和交互模式；遇到登录/付费/不可读取时提示一句，可由用户补截图/链接，也可跳过。
- **结构化品牌规范**：涉及品牌时同时写 `brand-spec.md` 和 `brand-spec.json`，HTML 从 JSON 引用 logo、产品图、UI 截图、色彩和字体。
- **Design lint**：交付前运行 `scripts/design_lint.py`，检查 assumptions、真实资产引用、AI slop 风险和品牌 spec 使用情况。
- **Definition of Done**：每个产物都要 lint + Playwright 验证，warning 要修复或解释。
...
涉及具体品牌时强制执行：问 → 搜 → 下载（三条兜底）→ grep 色值 → 写 `brand-spec.md` + `brand-spec.json`。
...
| 5 · 固化 spec | 写 `brand-spec.md` + `brand-spec.json` + CSS 变量，所有 HTML 引用 `var(--brand-*)` | 不固化就会忘 |

A/B 测试（v1 vs v2，各跑 6 agent）：**v2 的稳定性方差比 v1 低 5 倍**。稳定性的稳定性，这是 skill 真正的护城河。

### 任务路由器 + 完成定义

每个任务先走 `references/task-router.md`。它规定输出类型、必读 reference、必用 starter assets/scripts，以及 Definition of Done。路由器不是捷径；它的作用是防止 agent 把所有需求都做成同一种网页。
...
- 交付前跑 `scripts/design_lint.py` + Playwright，肉眼过一遍浏览器
...
其中一部分已经被固化成 `scripts/design_lint.py`，用于交付前抓出常见 slop 和流程缺口。
...
│   ├── task-router.md        # 任务路由 + Definition of Done
│   ├── reference-intake.md    # 自动参考采样协议
│   ├── inspiration-sources.md # 默认参考源分类
│   ├── brand-spec-structured.md
│   ├── environment-adapters.md
│   ├── demo-index.md         # demos/*.html 与任务路由的映射
│   ├── asset-inventory.md    # 动态资产 / 保留 / 删除记录
...
│   └── ...
├── scripts/                 # 导出工具链
│   ├── design_lint.py       # 设计流程 + AI slop 静态检查
...
│   ├── html2pptx.js
│   └── verify.py
├── templates/                # Junior pass 起手模板，不能直接当成品
│   ├── landing-page.html
│   ├── app-overview.html
│   ├── app-flow.html
│   ├── deck-showcase.html
│   ├── motion-stage.html
│   └── infographic.html
└── demos/                   # 能力演示源码，按 demo-index.md 作为参考样例使用
```
